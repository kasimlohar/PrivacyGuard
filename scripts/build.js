...
function buildInterceptor() {
  const timestamp = new Date().toISOString();
  // Only lightweight regex modules for the MAIN world API boundary
  const INTERCEPTOR_SOURCES = [
    'src/utils/constants.js',
    'src/detection/regexEngine.js',
    'src/detection/injectionScanner.js',
    'src/utils/masker.js',
  ...
  let bundle = '';
  bundle += `// PrivacyGuard Interceptor Bundle\n`;
  bundle += `// Auto-generated by scripts/build.js — DO NOT EDIT DIRECTLY\n`;
  bundle += `// Built: ${timestamp}\n\n`;
  bundle += `(function () {\n`;
  bundle += `  'use strict';\n\n`;

  for (const file of INTERCEPTOR_SOURCES) {
    const fullPath = path.resolve(ROOT, file);
    let code = fs.readFileSync(fullPath, 'utf-8');
    code = stripModuleSyntax(code);
  ...
  const distDir = path.resolve(ROOT, 'dist');
  const outPath = path.resolve(distDir, 'interceptor.bundle.js');
  fs.writeFileSync(outPath, bundle, 'utf-8');

  const sizeKB = (Buffer.byteLength(bundle) / 1024).toFixed(1);
  console.log(`✅ Built: dist/interceptor.bundle.js (${sizeKB} KB)`);
}
